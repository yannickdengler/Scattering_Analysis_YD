...
def create_scattering(filename,hdfpath="./output/HDF5_logfiles/"):
    ...
    with open(filename.strip()) as fi:                          # .strip() removes a potential "\n" in the end of the string
        ...
        print("Size of Correlator array [num_Operators][num_sources][num_Montecarlotimes][N_T]:[%i][%i][%i][%i]"%(len(Correlators),len(Correlators[0]),len(Correlators[0][0]),len(Correlators[0][0][0])) )
        num_Montecarlotimes = len(Correlators[0][0])
        num_src = len(Correlators[0])


        os.makedirs(hdfpath, exist_ok=True)
        with h5py.File(hdfpath+"Scattering%s_%s_beta%1.3f_m1%1.3f_m2%1.3f_T%i_L%i_logfile.hdf5"%(isospin_str, gauge_group, beta, m_1, m_2, N_T, N_L),"w") as f:
        ...
        with h5py.File(hdfpath+"Scattering_src%s_%s_beta%1.3f_m1%1.3f_m2%1.3f_T%i_L%i_logfile.hdf5"%(isospin_str, gauge_group, beta, m_1, m_2, N_T, N_L),"w") as f:
            f.create_dataset("logfile name", data=logfile_name)
            f.create_dataset("isospin_channel", data=isospin_channel)
            f.create_dataset("N_mont", data = num_Montecarlotimes)
            f.create_dataset("N_hits", data = num_src)
            ...
            f.create_dataset("N_L", data = N_L)
            f.create_dataset("N_T", data = N_T)
            f.create_dataset("correlators", data = np.mean(Correlators, axis = 1))
        print()
