...
def resampling_JK_SAMEDIM(orig_sample, sampling_args):
    """
    Resamples an orig_sample with the cut-1 Jackknife method with more than one Observable coming from the same gauge configurations
    """
        ...
        for j in range(num_JK):
            if i != j:
                for k in range(num_obs):
                    Resamples[i][k] += orig_sample[k][j]/num_JK
    return Resamples
